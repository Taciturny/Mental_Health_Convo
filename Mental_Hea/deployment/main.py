...
from src.core.utils import initialize_qdrant, load_and_embed_data, upload_data_to_qdrant, is_relevant_query
import logging
...
def run():
    ...
    try:
        ...
        if not qdrant_manager.collection_exists(settings.COLLECTION_NAME_CLOUD):
            logger.info(f"Collection '{settings.COLLECTION_NAME_CLOUD}' does not exist. Creating it now.")
            qdrant_manager.create_collection(settings.COLLECTION_NAME_CLOUD)
            logger.info(f"Collection '{settings.COLLECTION_NAME_CLOUD}' created successfully.")
        ...
        if qdrant_manager.collection_is_empty(settings.COLLECTION_NAME_CLOUD):
            ...
            upload_data_to_qdrant(qdrant_manager, settings.COLLECTION_NAME_CLOUD, data, dense_embeddings, late_embeddings)
        ...
        while True:
            ...
            try:
                ...
                dense_results = search_engine.search_dense(query_text=query)  # you can change to any search
                ...
                cohere_response = cohere_model.generate_response(cohere_prompt, max_tokens=300, temperature=0.7)
        ...
        print("An error occurred while starting the application. Please check the logs and try again.")
