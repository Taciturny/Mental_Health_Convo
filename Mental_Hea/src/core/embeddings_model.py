...
class EmbeddingsModel:
    ...
    def _initialize(self):
        try:
            ...
            logger.error(f"Error initializing embeddings models: {str(e)}", exc_info=True)
            raise

    def embeddings(self, inputs: List[str], batch_size: int = 32) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        """
