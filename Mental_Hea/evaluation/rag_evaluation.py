...
def load_datasets():
    original_data = pd.read_parquet(settings.DATA_FILE_PATH).sample(n=4000, random_state=42).to_dict(orient='records')  # use a sample size of 4000 or less to save time
    ground_truth = pd.read_parquet(settings.GROUND_TRUTH_DATA).to_dict(orient='records')
    return original_data, ground_truth

def perform_dense_search(search_engine: SearchEngine, query: str, k: int = 5) -> List[Dict]:
...
def calculate_ndcg(relevance_scores: List[int], k: int) -> float:
    dcg = sum((2**rel - 1) / np.log2(i + 1) for i, rel in enumerate(relevance_scores[:k], 1))
    ideal_relevance = sorted(relevance_scores, reverse=True)
    idcg = sum((2**rel - 1) / np.log2(i + 1) for i, rel in enumerate(ideal_relevance[:k], 1))
...
def evaluate_model(model_name: str, ensemble_model: EnsembleModel, original_data: List[Dict], ground_truth: List[Dict], search_engine: SearchEngine, embeddings_model: EmbeddingsModel, batch_size: int = 100) -> List[Dict]:
    results = []
    
    # Pre-compute ground truth dictionary for faster lookup
    gt_dict = {gt['id']: gt for gt in ground_truth}

    for i in tqdm(range(0, len(original_data), batch_size), desc=f"Evaluating {model_name}"):
        batch = original_data[i:i+batch_size]
        batch_results = []

        # Perform dense search for all items in the batch
        questions = [item['question'] for item in batch]
        batch_retrieved_results = [perform_dense_search(search_engine, q) for q in questions]

        # Generate LLM answers for all items in the batch
        prompts = []
        for item, retrieved in zip(batch, batch_retrieved_results):
            context = ' '.join([f"Q: {r.get('question', '')} A: {r.get('answer', '')}" for r in retrieved])
        ...
        if model_name == 'ensemble':
            ...
            batch_generated_responses = ensemble_model._generate_single_model(model_name, prompts)

        # Process each item in the batch
        for item, retrieved_results, answer_llm in zip(batch, batch_retrieved_results, batch_generated_responses):
            original_id = item['id']
            ...
            if original_id in gt_dict:
                ...
                for result in retrieved_results:
                    result_embedding, _ = embeddings_model.embeddings([result['answer']])
                    similarity = calculate_cosine_similarity(original_embedding[0], result_embedding[0])
                    relevance_scores.append(1 if similarity > 0.8 else 0)

                relevant_retrieved = sum(relevance_scores)
                precision = relevant_retrieved / len(retrieved_results) if retrieved_results else 0
                ...
                cosine_sim = calculate_cosine_similarity(original_embedding[0], llm_embedding[0])
                ...
                batch_results.append({
                    ...
                    'search_score': retrieved_results[0]['score'] if retrieved_results else None
                })
...
def main():
    ...
    with mlflow.start_run(run_name="model_comparison_evaluation"):
        ...
        for model_name in tqdm(['gpt2', 'dialogpt', 'distilgpt2'], desc="Models"):
            ...
            model_results = evaluate_model(model_name, ensemble_model, original_data, ground_truth, search_engine, embeddings_model)
            ...
            avg_metrics = {
                ...
                'cosine_similarity': np.mean([r['cosine_similarity'] for r in model_results])
            }
