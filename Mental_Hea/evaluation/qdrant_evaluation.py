...
class QdrantRetrievalEvaluator:
    ...
    def evaluate(self, k: int = 5):
        ...
        for search_type in search_types:
            mlflow.set_experiment(f"Qdrant Retrieval Evaluation - {search_type.capitalize()}")
            ...
            with mlflow.start_run():
                ...
                for query_id, query_text in self.ground_truth.items():
                    ...
                    retrieved_scores = [point.score for point in search_results.points[:k]]

                    metrics = self.calculate_metrics(retrieved_docs, retrieved_scores, relevant_docs, k)
                    for metric, value in metrics.items():
                        results[search_type][metric].append(value)

                mean_metrics = {metric: np.mean(values) for metric, values in results[search_type].items()}
    ...
    def calculate_metrics(self, retrieved_docs: List[str], retrieved_scores: List[float], relevant_docs: set, k: int) -> Dict[str, float]:
    ...
    @staticmethod
    def ndcg(retrieved_docs: List[str], relevant_docs: set, scores: List[float], k: int) -> float:
    ...
    def save_results_to_csv(self, results: Dict[str, Dict[str, List[float]]], k: int):
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            for search_type, metrics in results.items():
                row = [search_type] + [f"{np.mean(values):.4f}" for values in metrics.values()]
                writer.writerow(row)
