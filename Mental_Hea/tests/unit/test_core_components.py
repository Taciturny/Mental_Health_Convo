...
from src.core.utils import initialize_qdrant, load_and_embed_data, is_relevant_query
...
class TestDataLoader(unittest.TestCase):
    ...
    def test_load_data_in_batches(self, mock_read_parquet):
        mock_df = MagicMock()
        mock_df.__len__.return_value = 10
        mock_df.iloc.__getitem__.return_value.to_dict.return_value = [{"id": 1, "question": "Test?", "answer": "Answer"}]
        mock_read_parquet.return_value = mock_df
...
class TestEmbeddingsModel(unittest.TestCase):
    ...
    def test_embeddings(self, mock_late, mock_dense):
        mock_dense_output = np.array([0.025, 0.019, -0.059, 0.006, -0.060, 0.033, 0.018, 0.007, 0.056, 0.073, 0.030, -0.069], dtype=np.float32)
        mock_late_output = np.array([[0.100, 0.013, -0.109, -0.023, 0.098, 0.128, -0.038, 0.003]], dtype=np.float32)
...
class TestQdrantManager(unittest.TestCase):
    ...
    def test_init_with_host_port(self, mock_qdrant_client):
        manager = initialize_qdrant("test_collection")
    
        # Ensure that QdrantManager was initialized with localhost and the correct port
        mock_qdrant_client.assert_called_once_with(host="localhost", port=6333, timeout=600)
    ...
    def test_init_with_url_api_key(self, mock_settings, mock_qdrant_client):
        ...
        mock_qdrant_client.assert_called_once_with(url=mock_settings.QDRANT_URL, api_key=mock_settings.QDRANT_API_KEY, timeout=600)
    ...
    def test_create_collection_if_not_exists(self, mock_create_collection, mock_collection_exists, mock_qdrant_client):
        manager = QdrantManager(self.host, self.port)
        collection_name = "test_collection"
...
class TestSearchEngine(unittest.TestCase):
    ...
    def test_search_dense(self, mock_embeddings, mock_client):
        mock_embeddings.get_instance.return_value.embeddings.return_value = ([np.array([1, 2, 3])], None)
        ...
        result = self.search_engine.search_dense("test query")
    ...
    def test_search_hybrid(self, mock_embeddings, mock_client):
        mock_embeddings.get_instance.return_value.embeddings.return_value = ([np.array([1, 2, 3])], [np.array([4, 5, 6])])
        mock_client.return_value.query_points.return_value = MagicMock()
        
        # Replace the search engine's client with the mock
        self.search_engine.client = mock_client.return_value
        
        result = self.search_engine.search_hybrid("test query")
...
class TestUtils(unittest.TestCase):
    ...
    def test_initialize_qdrant(self, mock_qdrant_manager):
        ...
        mock_qdrant_manager.return_value.create_collection_if_not_exists.assert_called_once_with("test_collection")
    ...
    def test_load_and_embed_data(self, mock_embeddings_model, mock_data_loader):
        mock_data_loader.return_value.load_data_in_batches.return_value = [[{"question": "Test?"}]]
        mock_embeddings_model.get_instance.return_value.embeddings.return_value = ([np.array([1, 2, 3])], [np.array([4, 5, 6])])
        
        data, dense_embeddings, late_embeddings = load_and_embed_data("test_file.parquet")
    ...
    def test_keyword_match(self, mock_sentiment_analyzer):
        ...
        self.assertTrue(is_relevant_query("Looking for anxiety treatment", relevant_keywords))
        
        # Test non-matching query
        self.assertFalse(is_relevant_query("What's the weather like today?", relevant_keywords))
    ...
    def test_sentiment_analysis(self, mock_sentiment_analyzer):
        ...
        relevant_keywords = ["depressed", "anxiety", "treatment"]
        self.assertTrue(is_relevant_query("I feel terrible and hopeless", relevant_keywords))
        ...
        self.assertFalse(is_relevant_query("The weather is a bit gloomy", relevant_keywords))
