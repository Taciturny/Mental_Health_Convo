...
class DataPreprocessor:
    ...
    def normalize_and_clean_text(self, df):
        ...
        for col in ['input', 'output']:
            df[col] = df[col].apply(lambda text: self.remove_irrelevant_content(text.lower().strip()))
        return df
    ...
    def save_data(self, df, output_path):
        ...
        print(f"Data saved to {output_path} with columns: {', '.join(processed_data.columns)}")
...
def main(parquet_path, csv_path, output_path):
    ...
    df_csv_processed = preprocessor.preprocess_data(df_csv.drop(columns=['Question_ID'], errors='ignore'))
    ...
    print(f"Average responses per input: {combined_df.groupby('question').size().mean():.2f}")
